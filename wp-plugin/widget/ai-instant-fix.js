...
(function () {
  ...
  var completedIds = {};  // track seen completed task IDs for toast
  try { completedIds = JSON.parse(localStorage.getItem('aif_completed') || '{}'); } catch (e) {}

  function markCompleted(taskId) {
    completedIds[taskId] = true;
    try { localStorage.setItem('aif_completed', JSON.stringify(completedIds)); } catch (e) {}
  }
  ...
  // ── Toast notification ────────────────────────────
  function showToast(taskId, prompt) {
    var t = document.getElementById('aif-toast');
    if (!t) { buildToast(); t = document.getElementById('aif-toast'); }
    t.querySelector('.aif-toast-text').textContent = 'Task #' + taskId + ' selesai';
    t.classList.add('show');
    // Auto-hide after 30s
    setTimeout(function () { t.classList.remove('show'); }, 30000);
  }
  ...
  function buildToast() {
    ...
    t.id = 'aif-toast';
    t.innerHTML =
      '<span class="aif-toast-text"></span>' +
      '<button class="aif-toast-reload">Reload page</button>' +
      '<button class="aif-toast-close">&times;</button>';
    t.querySelector('.aif-toast-reload').onclick = function () { window.location.reload(); };
    t.querySelector('.aif-toast-close').onclick = function () { t.classList.remove('show'); };
  ...
  function loadTasks() {
    api('GET', 'list', { page_url: C.url, limit: 50 })
      .then(function (d) {
        var newTasks = d.tasks || [];
        // Detect newly completed tasks for toast
        for (var i = 0; i < newTasks.length; i++) {
          var t = newTasks[i];
          if (t.status === 'task completed' && !completedIds[t.id]) {
            markCompleted(t.id);
            showToast(t.id, t.prompt);
          }
        }
        tasks = newTasks; render();
      })
